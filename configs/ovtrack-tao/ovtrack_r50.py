# 1. model
# 1.1 each part
# 1.2 detector: model + train_cfg + test_cfg
model = dict(
    type='OVTrack',
...
"""
    2. dataset -> inherited from CustomDataset
    2.1 __init__: images annotations(__len__)
        2.1.1 prepare annotations in the format of object dict
        2.1.2 return image annotations(img_infos) dict to initialize __len__ and __getitem__
            2.1.2.1 video data: vids, vidToImgs(then using ·imgToAnns· to get annotations)
            2.1.2.2 image data: image_level_annotation, category_level_annotation, imgs
    2.2 __getitem__: from img_info dict
        2.2.1 prepare_train_img
        2.2.2 prepare_test_img
    2.3 pipelines (transform.Compose([]))
        2.3.1 train pipeline
        2.3.2 test pipeline
    2.4 DatasetWrapper(ClassBalancedDataset、SeqMultiImageMixDataset...)
"""
