...
def majority_vote(prediction):
    ...
    for tid, group in tqdm.tqdm(groued_df_pred_res):
        cid = group["category_id"].mode()[0] # find common cid
...
def sample_additional_ids(original_ids, selected_ids, ratio, seed=42):
    # 确保输入的是列表
    original_ids = list(original_ids)
    selected_ids = list(selected_ids)

    # 计算需要额外采样的数量
    additional_count = int(len(selected_ids) * ratio)

    # 创建可选择的id池 (排除已选择的ids)
    available_ids = list(set(original_ids) - set(selected_ids))

    # 如果可用的ids数量不足,则返回所有可用的
    if len(available_ids) < additional_count:
        return available_ids

    # # 随机抽样
    # sampled_ids = random.sample(available_ids, additional_count)
...
class TaoDataset(CocoVideoDataset):
    ...
    def load_tao_anns(self, ann_file):
        ...
        for vid_id in self.vid_ids:
            ...
            if self.is_select_ori_img:
                img_ids = [ imid for imid in img_ids if self.coco.load_imgs([imid])[0]['is_ori']==True]
                if self.extra_sample_ratio > 0:
                    extra_img_ids = sample_additional_ids(ori_img_ids, img_ids, self.extra_sample_ratio)
                    img_ids.extend(extra_img_ids)
    ...
    def _track2json(self, results):
        ...
        results = [results[inds[i] : inds[i + 1]] for i in range(num_vids)] # divid to videos, continuous sequences
            ...
            for img_info, result in zip(_img_infos, _results):
                ...
                for label in range(len(result)):
                    ...
                    for i in range(bboxes.shape[0]):
                        ...
                        if bboxes[i].shape[0] == 7:
                            data['prompt_prob'] = float(bboxes[i][-2])
    ...
    def evaluate(
        ...
        if "track" in metrics and not use_tao_metric:
            ...
            default_eval_config["NUM_PARALLEL_CORES"] = 16 # 8
            ...
            combined_result = format_teta_result(eval_res['COMBINED_SEQ']['average']['TETA'][50], 'Combined', ignore_title=False)
            teta_metric_list = ["T"]
            ...
            print_log('\n'+combined_result+base_result+novel_result, logger)
