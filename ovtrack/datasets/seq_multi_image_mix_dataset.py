...
class SeqMultiImageMixDataset:
    ...
    def __getitem__(self, idx):
        while True:
            ...
            # print(f"[{rank}/{world_size}] [3] seq_multi_image after inner:"+" " + results[0]['img_info']['filename'], results[1]['img_info']['filename'])
            ...
            for (transform, transform_type) in zip(self.pipeline,
                ...
                try:
                    ...
                    print(f"Error data index is {idx}")
            ...
            if isinstance(results, list):
                if len(results[0]['gt_labels'].data) == 0 or len(results[1]['gt_labels'].data) == 0 or len(results[0]['ref_gt_labels'].data) == 0 or len(results[1]['ref_gt_labels'].data) == 0:
                    idx = self._rand_another(idx)
                    continue
            # print(f"[{rank}/{world_size}] [4] seq_multi_image before train"+" " + results['img_metas'].data['filename'], results['ref_img_metas'].data['filename'])
            return results
